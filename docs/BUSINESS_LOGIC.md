...
### 10.1. Cấu trúc tổ chức Branch và Department

**Trạng thái: Đã thống nhất cho thiết kế database nền tảng**

- **Branch** đại diện cho một chi nhánh hoặc địa điểm vận hành của công ty. Branch được dùng để tổ chức Staff, giới hạn data scope và tổng hợp báo cáo theo chi nhánh.
- **Department** đại diện cho một phòng ban chức năng cụ thể trong một Branch, ví dụ Sales, Pricing, Operations, Customer Service, Accounting hoặc Administration.
- **Team** đại diện cho một nhóm làm việc cụ thể trong một Department, ví dụ Sales Enterprise, Sales SME hoặc Operations Air Freight.
- Một Branch có thể có nhiều Department; mỗi Department thuộc đúng một Branch.
- Một Department có thể có nhiều Team; mỗi Team thuộc đúng một Department.
- Mã Department chỉ cần duy nhất trong phạm vi Branch. Vì vậy các chi nhánh khác nhau có thể cùng sử dụng mã như `SALES` hoặc `OPS`.
- Mã Team chỉ cần duy nhất trong phạm vi Department.
- Không lưu thêm `branch_id` trong Team vì Branch đã được xác định thông qua Department; điều này tránh dữ liệu tổ chức bị trùng hoặc mâu thuẫn.
- Branch, Department và Team phải được ngừng hoạt động hoặc soft delete thay vì xóa cứng khi đã được dữ liệu nghiệp vụ tham chiếu.
- Không cho phép xóa cứng Branch khi Branch vẫn còn Department.
- Không cho phép xóa cứng Department khi Department vẫn còn Team.

Phân biệt các khái niệm tổ chức và phân quyền:

```text
Branch     = Staff và hoạt động thuộc chi nhánh nào
Department = Staff thuộc bộ phận chuyên môn nào
Team       = Staff làm việc trong nhóm nào
Role       = Staff được phép thực hiện những hành động nào
```
...
| Ngày | Nội dung |
|---|---|
| 2026-07-13 | Thống nhất Team thuộc một Department, mã Team duy nhất trong từng Department và không lưu dư thừa `branch_id` tại Team. |
| 2026-07-13 | Thống nhất ý nghĩa và ranh giới database của Branch và Department; xác định Department thuộc một Branch, mã Department duy nhất trong từng Branch và tách biệt Department với Team/Role. |
